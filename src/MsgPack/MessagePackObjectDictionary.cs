...
namespace MsgPack
{
		...
		IDictionary<MessagePackObject, MessagePackObject>, IDictionary
	{
		...
		ICollection<MessagePackObject> IDictionary<MessagePackObject, MessagePackObject>.Keys => this.Keys;

		ICollection<MessagePackObject> IDictionary<MessagePackObject, MessagePackObject>.Values => this.Values;
		...
		bool ICollection<KeyValuePair<MessagePackObject, MessagePackObject>>.IsReadOnly => this.IsFrozen;

		[SuppressMessage("Microsoft.Design", "CA1033:InterfaceMethodsShouldBeCallableByChildTypes", Justification = "Child types should never call this property.")]
		bool IDictionary.IsFixedSize => this.IsFrozen;
		...
		object? IDictionary.this[object key]
		{
			get
			{
				...
				var typedKey = ValidateObjectArgument(key);
				if (typedKey.IsNil)
				{
					ThrowKeyNotNilException("key");
				}
		...
		{
			Ensure.IsNotLessThan(initialCapacity, 0);
		...
		private void AssertInvariant()
		{
			...
			{
				Debug.Assert(this._keys != null, "this._keys != null");
				Debug.Assert(this._values != null, "this._values != null");
				Debug.Assert(this._keys.Count == this._values.Count, "this._keys.Count == this._values.Count");
				Debug.Assert(
		...
		private static MessagePackObject? TryValidateObjectArgument(object? value)
		{
			...
			if (value is MessagePackString asMessagePackString)
			{
		...
		{
			Debug.Assert(!key.IsNil, "!key.IsNil");
		...
		void IDictionary.Add(object key, object? value)
		{
			...
			this.AddCore(typedKey, ValidateObjectArgument(value), false);
		}
		...
		void IDictionary.Remove(object key)
		{
			...
			if (typedKey.IsNil)
			{
				ThrowKeyNotNilException(nameof(key));
			}
		...
		{
			if (array is DictionaryEntry[] asDictionaryEntries)
			{
